...
- **Wi-Fi credentials.** macOS keeps saved Wi-Fi passwords in the root-owned System keychain, which third-party apps cannot read. CoreWLAN offers no way to ask macOS to join using its own stored password: `associateToNetwork:password:` documents the passphrase as *required* for WEP, WPA and WPA2 Personal, and `associateToEnterpriseNetwork:` requires 802.1X credentials. Storing credentials is therefore the only way to join without prompting every time. So that joining a saved network does not prompt every time, a password the user types into WireBar is stored in WireBar's own keychain item (service `com.scottkostolni.WireBar.wifi`), separate from the license item. A password typed into the popover is stored only after it has successfully connected; one entered in Settings is stored as given. WireBar never reads from or writes to the system Wi-Fi keychain, and Settings never displays a stored password, only replaces it. This is the single exception to the in-memory-only rule above.
  - For 802.1X (work or school) networks WireBar stores the **username and password** the user signs in with, which is an account credential rather than a shared key. Both are kept the same way, in the same item, and the user is told so at the point of entry and in Settings.
  - One keychain item per network, JSON-encoded so the username travels with the password. Entries written before usernames existed are bare strings and are read as a password with no username.
  - Stored in the **file-based** keychain: encrypted at rest, and each item ACL'd to the creating application's code signature, so another app reading it prompts the user. The modern data-protection keychain would be stronger (hard denial rather than a prompt, plus `WhenUnlockedThisDeviceOnly`), but it requires an `application-identifier` or `keychain-access-groups` entitlement that only a provisioning profile grants — a Developer ID build has neither and every write fails with `-34018 errSecMissingEntitlement`. Revisit if WireBar ever ships sandboxed or provisioned.
