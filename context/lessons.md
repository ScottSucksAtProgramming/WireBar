...
- 2026-09-15: A `@Published` sink with `.receive(on: DispatchQueue.main)` defers delivery to the next runloop turn, so a synchronous "I'm writing this myself" reentrancy flag is already cleared when the sink fires. Drop the hop when the guard matters and the writer is already on the main actor.
- 2026-09-15: This machine has only a "Developer ID Application" cert, no "Mac Development" one, so default Debug builds fail with `No signing certificate "Mac Development" found`. Build with `CODE_SIGN_STYLE=Manual CODE_SIGN_IDENTITY="Developer ID Application: Scott Kostolni (5N69HV7X7S)" DEVELOPMENT_TEAM=5N69HV7X7S`. Ad-hoc signing (`CODE_SIGNING_ALLOWED=NO`) compiles but is useless for testing anything signature-sensitive — TCC/Location grants and login-item registration key off the signature.
- 2026-09-15: `/Applications` is group-writable here, so installing a build needs no sudo.
- 2026-09-15: For Wi-Fi association bugs, `log stream --predicate 'process == "airportd"' --info` shows each client request as `BEGIN/END REQ [VERB] (pid= proc= ... err=)`, naming the calling app. Disassoc reason code 8 = "leaving BSS", i.e. a deliberate teardown — which is what a failed associate() leaves behind.
- 2026-09-15: To show panel-style intro text in a grouped `Form` without breaking settings-window sizing, put it in the first `Section`'s `header:` (not a `VStack` above the `Form`). Header text defaults to secondary; `.foregroundStyle(.primary)` + `.fontWeight(.regular)` overrides it.
- 2026-09-15: WireBar's saved Wi-Fi passwords live in the file-based login keychain, not the data-protection keychain Safari/Passwords use — don't tell users it's "the same place Safari stores passwords".
- 2026-09-15: Computer-use `request_access` can't find WireBar even when running and Spotlight-indexed (menu-bar-only app), and `@WireBar` in the prompt resolved to the source folder. Fall back to Scott checking manually.
- 2026-09-15: Beta license keys must be stored under their own keychain account (`beta_license_key`). `restoreFromKeychain` kicks off LemonSqueezy `validateLicense()`, which clears credentials when the API says a key is invalid — a beta key under `license_key` would be wiped on every launch.
- 2026-09-15: zsh aborts the whole `;`-chained command when a glob has no match (`no matches found`), so later steps silently never run. Don't put unquoted globs in multi-step verification commands.
- 2026-09-15: Tests that build `LicenseManager()` with the default keychain read the developer's real login keychain. Once a real beta key was entered, 8 "blocked for free users" tests failed. Tests must pass `InMemoryKeychainStorage()`.
- 2026-09-15: The flaky `LicenseManagerTests` crash (run stops at 91 tests) was a data race in the test fakes, not the app. `LicenseManager` init starts a background `validateLicense()` Task while tests call it too; unlocked `MockLicenseValidator` counters and `InMemoryKeychainStorage` collided. Thread Sanitizer found it in one run. Test fakes shared with background work need a lock.
- 2026-09-15: Release pipeline proven end to end: `scripts/release.sh <version>` on main → Scott answers y → GitHub release; 0.2.0-beta → 0.2.1-beta updated in-app. The publish prompt needs a real terminal (`read -q` with no TTY = no), so Scott runs releases himself. `dist/` must be gitignored as a whole — `appcast.xml` and notes HTML aren't `*.dmg`.
